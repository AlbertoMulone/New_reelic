# policy
resource "newrelic_alert_policy" "lcert_vaga_policy" {
  name = "lcert ${var.env_newrelic} vaga policy"
}
...
module "notification" {
  ...
  name_prefix  = "lcert ${var.env_newrelic} vaga"
  policy_id    = newrelic_alert_policy.lcert_vaga_policy.id
...
module "apm_v2" {
  ...
  policy_id     = newrelic_alert_policy.lcert_vaga_policy.id
  filter_app    = "( `appName` = 'vaga_infocert_${var.env_newrelic}' )"
  filter_trnsct = "( `appName` = 'vaga_infocert_${var.env_newrelic}' AND `transactionType` = 'Web' AND `httpResponseCode` NOT LIKE '4%' AND `request.uri` != '/sign/validation/dss/actuator/health' AND `request.uri` != '/sign/validation/dss/v1/web/document' )"
  enabled       = var.enabled
