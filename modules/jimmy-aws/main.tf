...
module "apm_v2" {
  ...
  filter_trnsct = "( `appName` = '${local.complete_appname}' AND `transactionType` = 'Web' AND `httpResponseCode` NOT LIKE '4%' AND `name` != 'WebTransaction/SpringController/OperationHandler/handle')"
  enabled       = var.enabled
