...
module "apm_v2" {
  ...
  filter_trnsct = "( `appName` = '${local.complete_appname}' AND `name` != 'WebTransaction/Vertx/health/ready (GET)' AND `httpResponseCode` NOT LIKE '4%' AND `transactionType` = 'Web' )"
  enabled       = var.enabled
