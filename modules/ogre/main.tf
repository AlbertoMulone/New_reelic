# policy
resource "newrelic_alert_policy" "lcert_ogre_policy" {
  name = "lcert ${var.env} ogre ${var.label} policy"
}
...
module "notification" {
  ...
  name_prefix  = "lcert ${var.env} ogre ${var.label}"
  policy_id    = newrelic_alert_policy.lcert_ogre_policy.id
...
# process
resource "newrelic_infra_alert_condition" "ogre_not_running" {
  policy_id = newrelic_alert_policy.lcert_ogre_policy.id

  name          = "ogre not running"
...
# no data
resource "newrelic_nrql_alert_condition" "ogre_no_data" {
  policy_id = newrelic_alert_policy.lcert_ogre_policy.id
...
SELECT count(*)
FROM OGRESample
WHERE `displayName` IN ('check_local', 'check_balancer')
AND `service_name` = '${var.service_name}'
AND `environment` IN ('${join("', '", var.network_envs)}')
