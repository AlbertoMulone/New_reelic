...
// Click on Templates
const templatesLink = await waitForAndFindElement($selenium.By.partialLinkText("Templates"), 3000)
await templatesLink.click().then(function(text){
	console.log('Templates clicked');
});

// Verification 
const infocertText = await waitForAndFindElement($selenium.By.xpath('//*[text() = "INFOCERT"]'), 6000)
...
// Logout1
const logout1Link = await $webDriver.findElement($selenium.By.partialLinkText("${service_name}"))
